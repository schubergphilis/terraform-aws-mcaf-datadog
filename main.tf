...
module "datadog_integration_role" {
  ...
  create_policy = true
  policy_arns   = var.cspm_resource_collection_enabled ? ["arn:aws:iam::aws:policy/SecurityAudit", "arn:aws:iam:::policy/${local.datadog_resource_collection_policy_name}"] : []
  postfix       = false
